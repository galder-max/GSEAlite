GSEA.plot<-function(m,
                    ...
                    col=rgb(.3,.73,.3))
  {
    ...
    scrandom<-lapply(1:nperms,function(x)
                     {
                       s2n<-if(is.null(o)) get(paste("snr.",metric,sep=""))(apply(m,2,function(x) sign(rnorm(1))*x),o)
    ...
    meanES<-mean(sapply(scrandom,function(x) x$res$ess))
    layout(mat=cbind(c(1,2,3)),widths=1,heights=c(6,1,4))
    par(mar=c(0,6,4,1))
    sc<-leading.edge(k=k,
                     forder=scoresorder,
                     meanES=meanES,
    ...
    par(mar=c(0,6,0,1))
    plot(0,0,col=rgb(0,0,0,0),ylim=c(-1,1),xlim=c(1,nrow(m)),frame.plot=F,xaxt="n",yaxt="n",ylab="",xlab="")
    abline(v=sc$rug,col=rgb(.2,.2,.2,.8))
    par(mar=c(4,6,0,1))
    plot(1:nrow(m),scores[order(scores,decreasing=T)],type="l",
         lwd=2,
         xlab="Rank",
         ylab=if(metric[1]=="SNR") "Signal to Noise" else "Fold Change",
